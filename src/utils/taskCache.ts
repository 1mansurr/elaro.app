...
interface ExtendedHomeScreenData {
  nextUpcomingTask?: Assignment | Lecture | StudySession | null;
...
export async function getCachedTask(
  ...
  if (queryClient) {
    ...
    if (cached && Array.isArray(cached)) {
      const task = cached.find(
        (t: Assignment | Lecture | StudySession) => t.id === taskId,
      );
      if (task) {
  ...
  try {
    const homeScreenData =
      await cache.get<ExtendedHomeScreenData>('homeScreenData');
    if (homeScreenData) {
...
export function mergeTaskUpdates(
  ...
  updates: Partial<Assignment | Lecture | StudySession>,
