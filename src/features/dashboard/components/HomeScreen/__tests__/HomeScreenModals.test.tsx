...
jest.mock('@/shared/components/TaskDetailSheet', () => {
  return function MockTaskDetailSheet({ isVisible, task }: any) {
    return isVisible ? (
      <div data-testid="task-detail-sheet">Task Detail Sheet</div>
    ) : null;
  };
});
...
jest.mock('@/shared/components', () => ({
  QuickAddModal: function MockQuickAddModal({ isVisible }: any) {
    return isVisible ? (
      <div data-testid="quick-add-modal">Quick Add Modal</div>
    ) : null;
  },
}));

jest.mock(
  '@/features/notifications/components/NotificationHistoryModal',
  () => {
    return function MockNotificationHistoryModal({ isVisible }: any) {
      return isVisible ? (
        <div data-testid="notification-history-modal">
