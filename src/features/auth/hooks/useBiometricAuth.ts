...
import { BiometricAuthService, BiometricAuthState } from '../services/BiometricAuthService';

export interface UseBiometricAuthReturn {
  biometricState: BiometricAuthState | null;
  loading: boolean;
  error: string | null;
  enableBiometricAuth: (user: User) => Promise<{ success: boolean; error?: string }>;
  disableBiometricAuth: () => Promise<void>;
  authenticateWithBiometric: (promptMessage?: string) => Promise<{ success: boolean; error?: string }>;
...
export const useBiometricAuth = (): UseBiometricAuthReturn => {
  const [biometricState, setBiometricState] = useState<BiometricAuthState | null>(null);
  ...
  const refreshBiometricState = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get biometric auth state';
  ...
  const enableBiometricAuth = useCallback(async (user: User) => {
    try {
      setError(null);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to enable biometric auth';
      setError(errorMessage);
  ...
  const disableBiometricAuth = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to disable biometric auth';
      setError(errorMessage);
  ...
  const authenticateWithBiometric = useCallback(async (promptMessage?: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to authenticate with biometrics';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    }
  }, [biometricAuthService]);
  ...
  const signInWithBiometric = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to sign in with biometrics';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    }
  }, [biometricAuthService]);
