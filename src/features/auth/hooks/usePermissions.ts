...
import { PermissionService, PermissionCheckResult, TaskLimitCheck } from '../permissions/PermissionService';
...
export interface UsePermissionsReturn {
  ...
  canCreateTask: (taskType: 'assignments' | 'lectures' | 'study_sessions') => Promise<PermissionCheckResult>;
...
export const usePermissions = (user: User | null): UsePermissionsReturn => {
  ...
  const hasPermission = useCallback(async (permission: Permission): Promise<PermissionCheckResult> => {
    if (!user) {
  ...
  const canCreateTask = useCallback(async (taskType: 'assignments' | 'lectures' | 'study_sessions'): Promise<PermissionCheckResult> => {
    if (!user) {
      return { allowed: false, reason: 'User not authenticated' };
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Task creation check failed';
      setError(errorMessage);
      return { allowed: false, reason: errorMessage };
    } finally {
      setLoading(false);
  ...
  const canCreateSRSReminders = useCallback(async (): Promise<PermissionCheckResult> => {
    if (!user) {
      return { allowed: false, reason: 'User not authenticated' };
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'SRS reminder check failed';
      setError(errorMessage);
      return { allowed: false, reason: errorMessage };
    } finally {
      setLoading(false);
  ...
  const getTaskLimits = useCallback(async (): Promise<TaskLimitCheck[]> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get task limits';
      setError(errorMessage);
      return [];
    } finally {
      setLoading(false);
