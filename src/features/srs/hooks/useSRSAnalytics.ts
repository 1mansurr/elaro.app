...
} from '../services/SRSAnalyticsService';
...
export const useSRSAnalytics = (): UseSRSAnalyticsReturn => {
  ...
  const generateLearningInsights = useCallback(
    async (userId: string): Promise<LearningInsights | null> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getPerformanceDashboard = useCallback(
    async (userId: string): Promise<PerformanceDashboard | null> => {
      try {
        setLoading(true);
        setError(null);
