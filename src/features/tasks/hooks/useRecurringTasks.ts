...
export const useRecurringTasks = (): UseRecurringTasksReturn => {
  ...
  const createPattern = useCallback(
    async (request: CreatePatternRequest): Promise<RecurringPattern | null> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getUserRecurringTasks = useCallback(
    async (userId: string): Promise<RecurringTask[]> => {
      try {
        setError(null);
  ...
  const getAvailablePatterns = useCallback(async (): Promise<
    RecurringPattern[]
  > => {
    try {
      setError(null);
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to get available patterns';
      setError(errorMessage);
  ...
  const updateRecurringTask = useCallback(
    async (
      ...
      try {
        ...
        return await recurringTaskService.updateRecurringTask(
          recurringTaskId,
          updates,
        );
      } catch (err) {
  ...
  const deleteRecurringTask = useCallback(
    async (recurringTaskId: string): Promise<void> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getGeneratedTasks = useCallback(
    async (recurringTaskId: string): Promise<GeneratedTask[]> => {
      try {
        setError(null);
        return await recurringTaskService.getGeneratedTasks(recurringTaskId);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to get generated tasks';
        setError(errorMessage);
  ...
  const generateNextTasks = useCallback(
    async (recurringTaskId: string): Promise<GeneratedTask[]> => {
      try {
        setLoading(true);
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to generate next tasks';
        setError(errorMessage);
  ...
  const createCommonPatterns = useCallback(async (): Promise<void> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to create common patterns';
      setError(errorMessage);
