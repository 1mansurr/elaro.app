...
export const useTaskDependencies = (): UseTaskDependenciesReturn => {
  ...
  const createTaskWithDependencies = useCallback(
    async (
      task: EnhancedTask,
      userId: string,
    ): Promise<EnhancedTask | null> => {
      try {
        ...
        return await taskDependencyService.createTaskWithDependencies(
          task,
          userId,
        );
      } catch (err) {
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to create task with dependencies';
  ...
  const getTaskDependencies = useCallback(
    async (
      ...
      try {
        setError(null);
        return await taskDependencyService.getTaskDependencies(
          taskId,
          taskType,
        );
      } catch (err) {
  ...
  const getDependentTasks = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to get dependent tasks';
        setError(errorMessage);
  ...
  const validateDependencies = useCallback(
    async (
      ...
      try {
        setError(null);
        return await taskDependencyService.validateDependencies(
          dependencies,
          userId,
        );
      } catch (err) {
