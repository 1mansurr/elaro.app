...
export interface UseAdvancedTemplatesReturn {
  createTemplate: (userId: string, request: CreateTemplateRequest) => Promise<AdvancedTaskTemplate | null>;
  getUserTemplates: (userId: string) => Promise<AdvancedTaskTemplate[]>;
  getPublicTemplates: (filters?: TemplateSearchFilters) => Promise<AdvancedTaskTemplate[]>;
  getSharedTemplates: (userId: string) => Promise<AdvancedTaskTemplate[]>;
  getTemplateCategories: () => Promise<TemplateCategory[]>;
  createTaskFromTemplate: (userId: string, request: CreateTaskFromTemplateRequest) => Promise<string | null>;
  ...
  shareTemplate: (userId: string, request: ShareTemplateRequest) => Promise<TemplateShare[]>;
  rateTemplate: (userId: string, request: RateTemplateRequest) => Promise<TemplateRating | null>;
  getTemplateRatings: (templateId: string) => Promise<TemplateRating[]>;
  getTemplateUsage: (templateId: string) => Promise<TemplateUsage[]>;
  updateTemplate: (templateId: string, updates: Partial<CreateTemplateRequest>) => Promise<AdvancedTaskTemplate | null>;
...
export const useAdvancedTemplates = (): UseAdvancedTemplatesReturn => {
  ...
  const getUserTemplates = useCallback(async (userId: string): Promise<AdvancedTaskTemplate[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getUserTemplates(userId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get user templates';
      setError(errorMessage);
  ...
  const getPublicTemplates = useCallback(async (filters?: TemplateSearchFilters): Promise<AdvancedTaskTemplate[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getPublicTemplates(filters);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get public templates';
      setError(errorMessage);
  ...
  const getSharedTemplates = useCallback(async (userId: string): Promise<AdvancedTaskTemplate[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getSharedTemplates(userId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get shared templates';
      setError(errorMessage);
  ...
  const getTemplateCategories = useCallback(async (): Promise<TemplateCategory[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getTemplateCategories();
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get template categories';
      setError(errorMessage);
  ...
  const createTaskFromTemplate = useCallback(async (
    ...
    try {
      ...
      return await advancedTemplateService.createTaskFromTemplate(userId, request);
    } catch (err) {
  ...
  const createTemplateFromTask = useCallback(async (
    taskId: string,
    taskType: 'assignment' | 'lecture' | 'study_session',
    templateName: string,
    description: string = '',
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create template from task';
      setError(errorMessage);
  ...
  const getTemplateRatings = useCallback(async (templateId: string): Promise<TemplateRating[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getTemplateRatings(templateId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get template ratings';
      setError(errorMessage);
  ...
  const getTemplateUsage = useCallback(async (templateId: string): Promise<TemplateUsage[]> => {
    try {
      setError(null);
      return await advancedTemplateService.getTemplateUsage(templateId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get template usage';
      setError(errorMessage);
  ...
  const updateTemplate = useCallback(async (
    ...
    try {
      ...
      return await advancedTemplateService.updateTemplate(templateId, updates);
    } catch (err) {
  ...
  const getTemplateStats = useCallback(async (templateId: string): Promise<{
    usageCount: number;
    averageRating: number;
    totalRatings: number;
    recentUsage: number;
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get template stats';
      setError(errorMessage);
