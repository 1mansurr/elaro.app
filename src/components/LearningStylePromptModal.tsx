...
export const LearningStylePromptModal: React.FC<LearningStylePromptModalProps> = ({
  visible,
  onClose,
  prompt,
  isGuest,
  ...
  const truncatedPrompt = prompt.slice(0, 400) + (prompt.length > 400 ? '...' : '');
  ...
  return (
    <Modal visible={visible} animationType="fade" transparent>
      <View style={styles.overlay}>
        <View style={[styles.container, { maxHeight: height * 0.7 }]}> {/* Reduced modal height */}
          ...
          <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={true}>
            ...
            <Text style={styles.body}>
              1. Copy the prompt below{isGuest ? ' (sign in for full access)' : ''}
            ...
            <View style={styles.promptCard}>
              <Text selectable style={styles.promptText}>{isGuest ? truncatedPrompt : prompt}</Text>
              <TouchableOpacity
                style={[styles.copyButton, !canCopy && styles.copyButtonDisabled]}
                onPress={handleCopy}
                disabled={!canCopy}
                accessibilityLabel={canCopy ? 'Copy prompt to clipboard' : 'Sign in to copy prompt'}
              >
                <Feather name={canCopy ? 'copy' : 'lock'} size={18} color={canCopy ? COLORS.primary : COLORS.gray400} />
                <Text style={[styles.copyButtonText, !canCopy && styles.copyButtonTextDisabled]}>
              ...
              {isGuest && (
                <View style={styles.lockedMessageBox}>
                  <Text style={styles.lockedMessage}>
                    ...
                    To access the full prompt and enable the Copy feature, please sign in or create an account.
                  </Text>
                </View>
              )}
            </View>
