...
Before making changes or after applying migrations, validate that `schema.sql` matches the database state:
...
This script verifies that foreign key constraints match migration expectations, particularly checking that CASCADE deletes are correctly removed where specified.
...
### ⚠️ Current Plan: Free Tier

**Recovery Objectives:**

- **RPO (Recovery Point Objective)**: 24 hours (requires daily manual backups)
- **RTO (Recovery Time Objective)**: 2 hours (with automated restore scripts)

**Important Notes:**

- Free tier has **no automatic backups** - manual backups are required
- Daily backups must be implemented to achieve 24-hour RPO
- Weekly restore drills recommended to ensure 2-hour RTO
- Consider upgrading to Pro plan for automatic daily backups

**Plan Upgrade Path:**

- **Pro Plan** ($25/month): Automatic daily backups, improves reliability
- **Team Plan**: Hourly backups + Point-in-Time Recovery for 1-hour RPO/RTO
...
#### Required Backups (Free Tier):

- ✅ **Daily**: Full backup at 2 AM (automated via cron/script) - **REQUIRED for 24h RPO**
- ✅ **Before Production Deployment**: Full backup
- ✅ **Before Major Migrations**: Schema backup
- ✅ **Before Bulk Data Operations**: Data backup
- ✅ **Weekly**: Full backup with compression to external storage
- ✅ **Monthly**: Full backup with encryption to long-term storage

#### Daily Backup Automation (Recommended):

```bash
# Add to crontab (crontab -e):
# Run daily backup at 2 AM
0 2 * * * cd /path/to/ELARO-app && ./scripts/backup-manager.sh full >> /var/log/elaro-backup.log 2>&1
```
