...
function checkCriticalPaths(coverage) {
  ...
  for (const [path, threshold] of Object.entries(CRITICAL_PATH_THRESHOLDS)) {
    ...
    if (minPct >= threshold) {
      console.log(
        `✅ ${path}: ${minPct.toFixed(1)}% >= ${threshold}% threshold`,
      );
      ...
      console.log(
        `   Details: lines=${linesPct.toFixed(1)}%, functions=${functionsPct.toFixed(1)}%, branches=${branchesPct.toFixed(1)}%, statements=${statementsPct.toFixed(1)}%`,
      );
...
function checkGlobalCoverage(coverage) {
  ...
  if (minPct >= GLOBAL_THRESHOLD) {
    console.log(
      `✅ Global coverage: ${minPct.toFixed(1)}% >= ${GLOBAL_THRESHOLD}% threshold`,
    );
    console.log(
      `   Details: lines=${linesPct.toFixed(1)}%, functions=${functionsPct.toFixed(1)}%, branches=${branchesPct.toFixed(1)}%, statements=${statementsPct.toFixed(1)}%`,
    );
...
function main() {
  ...
  console.log(
    `Critical Paths: ${passedCount} passed, ${failedCount} failed, ${notFoundCount} not found`,
  );
