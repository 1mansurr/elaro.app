...
async function getAuthToken() {
  ...
  if (!testPassword) {
    ...
    console.error('   Please set TEST_PASSWORD in .env or sign in via the app first');
    process.exit(1);
  }
...
async function testStartTrial() {
  ...
  try {
  const { data, error } = await supabase.functions.invoke('start-user-trial', {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
if (command === 'check-username') {
  if (!arg) {
    console.error('❌ Usage: node scripts/test-edge-functions.js check-username <username>');
    process.exit(1);
  }
  ...
  console.error('\nAvailable commands:');
  console.error('  check-username <username>  - Test username availability check');
