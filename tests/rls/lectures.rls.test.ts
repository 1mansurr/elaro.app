...
describe('RLS: lectures permissions', () => {
  ...
  describe('READ permissions', () => {
    it('user1 cannot see user2 lectures', async () => {
      // sign in user2, create lecture
      await client.auth.signInWithPassword({ email: user2Email, password });
      ...
      const createdRes = await client
        ...
        .insert({
          ...
          lecture_name: 'u2-lecture',
        })
        .select()
        .single();
      if (createdRes.error) throw createdRes.error;

      // sign in user1, try to read user2 lecture
      await client.auth.signInWithPassword({ email: user1Email, password });
    ...
    it('user can see their own lectures', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
  ...
  describe('UPDATE permissions', () => {
    it('user1 cannot update user2 lectures', async () => {
      // user2 creates lecture
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
        .from('lectures')
    ...
    it('user can update their own lectures', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
      const currentUserRes = await client.auth.getUser();
      const createdRes = await client
        .from('lectures')
      ...
      const updateRes = await client
        .from('lectures')
        .update({ lecture_name: 'updated-name' })
        .eq('id', createdRes.data.id)
        .select()
        .single();

      expect(updateRes.error).toBeNull();
      expect(updateRes.data?.lecture_name).toBe('updated-name');
    }, 30000);
  });

  describe('DELETE permissions', () => {
    it('user1 cannot delete user2 lectures', async () => {
      // user2 creates lecture
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
      ...
      // Verify lecture still exists (user2 can still see it)
      await client.auth.signInWithPassword({ email: user2Email, password });
    ...
    it('user can delete their own lectures', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
      const currentUserRes = await client.auth.getUser();
      const createdRes = await client
        .from('lectures')
  ...
  describe('Soft delete visibility', () => {
    it('soft-deleted lectures are not visible to other users', async () => {
      // user2 creates lecture
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
  ...
  describe('Unauthenticated access', () => {
    it('unauthenticated user cannot access lectures', async () => {
      await client.auth.signOut();

      const rowsRes = await client.from('lectures').select('*');
