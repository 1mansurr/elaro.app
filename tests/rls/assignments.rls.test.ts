...
describe('RLS: assignments permissions', () => {
  ...
  describe('READ permissions', () => {
    it('user1 cannot see user2 assignments', async () => {
      // sign in user2, create assignment
      ...
      // sign in user1, try to read user2 assignment
    ...
    it('user can see their own assignments', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
  ...
  describe('UPDATE permissions', () => {
    it('user1 cannot update user2 assignments', async () => {
      // user2 creates assignment
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
        .from('assignments')
    ...
    it('user can update their own assignments', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
      const currentUserRes = await client.auth.getUser();
      const createdRes = await client
        .from('assignments')
  ...
  describe('DELETE permissions', () => {
    it('user1 cannot delete user2 assignments', async () => {
      // user2 creates assignment
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
      ...
      // Verify assignment still exists (user2 can still see it)
      await client.auth.signInWithPassword({ email: user2Email, password });
    ...
    it('user can delete their own assignments', async () => {
      await client.auth.signInWithPassword({ email: user1Email, password });
      const currentUserRes = await client.auth.getUser();
      const createdRes = await client
        .from('assignments')
        .insert({ user_id: currentUserRes.data.user?.id, title: 'to-delete' })
        .select()
        .single();
      if (createdRes.error) throw createdRes.error;
  ...
  describe('Soft delete visibility', () => {
    it('soft-deleted assignments are not visible to other users', async () => {
      // user2 creates assignment
      await client.auth.signInWithPassword({ email: user2Email, password });
      const currentUserRes2 = await client.auth.getUser();
      const createdRes = await client
  ...
  describe('Unauthenticated access', () => {
    it('unauthenticated user cannot access assignments', async () => {
      await client.auth.signOut();

      const rowsRes = await client.from('assignments').select('*');
