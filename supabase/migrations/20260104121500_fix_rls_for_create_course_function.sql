-- Fix: Add SECURITY DEFINER to create_course_and_lectures_transaction function
-- This allows the function to bypass RLS when inserting lectures, courses, and reminders
-- The function is called from an authenticated Edge Function that validates the user
...
BEGIN
    -- Security check: Ensure the authenticated user matches the provided user_id
    -- This prevents users from creating courses for other users
    IF auth.uid() IS DISTINCT FROM p_user_id THEN
        RAISE EXCEPTION 'User ID mismatch: cannot create course for different user';
    END IF;
    ...
    IF p_recurrence_type = 'none' THEN
        ...
        LOOP
            ...
            -- Include day_number and reminder_type
