-- Fix: Add lecture_date to create_course_and_lectures_transaction function
-- The lecture_date column is NOT NULL but was missing from INSERT statements
-- This migration adds lecture_date to both single and recurring lecture inserts
...
BEGIN
    ...
    IF p_recurrence_type = 'none' THEN
        ...
        -- FIX: Include lecture_date (set to start_time for backward compatibility)
    ...
    ELSE
        ...
        FOR i IN 0..15 LOOP
            ...
            -- FIX: Include lecture_date (set to current_lecture_start for backward compatibility)
            INSERT INTO public.lectures (user_id, course_id, lecture_name, start_time, end_time, venue, lecture_date)
