  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (!isRetryableError(error)) {
        console.log(`❌ Non-retryable error encountered: ${err.message || 'Unknown error'}`);
        throw error;
      }
      ...
      console.log(`⏳ Retry attempt ${attempt + 1}/${maxRetries} after ${delayWithJitter}ms (error: ${err.message || 'Unknown error'})`);
