// FILE: supabase/functions/restore-study-session/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      await checkRateLimit(supabase, user.id, 'restore-study-session');
    } catch (error) {
    ...
    return new Response(
      JSON.stringify({ message: 'Study session restored successfully.' }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      }
