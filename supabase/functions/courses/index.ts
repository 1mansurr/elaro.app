...
class CourseService {
  ...
  async createCourse(data: any, userId: string) {
    ...
    const courseLimit = courseLimits[userTier as keyof typeof courseLimits] || courseLimits.free;
  ...
  async deleteCourse(id: string, userId: string) {
    ...
    const { data, error } = await this.supabaseClient.rpc('soft_delete_record_cascade', {
      table_name: 'courses',
      record_id: id,
      user_id: userId,
  ...
  async restoreCourse(id: string, userId: string) {
    const { data, error } = await this.supabaseClient.rpc('restore_soft_deleted_record', {
      table_name: 'courses',
      record_id: id,
      user_id: userId
    });
...
async function handleCoursesRequest({ user, supabaseClient, body, url }: any) {
  ...
  if (method === 'GET' && path.endsWith('/courses')) {
    const includeDeleted = new URL(url).searchParams.get('include_deleted') === 'true';
  ...
  if (method === 'POST' && path.includes('/courses/') && path.includes('/restore')) {
