// FILE: supabase/functions/restore-assignment/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      await checkRateLimit(supabase, user.id, 'restore-assignment');
    } catch (error) {
    ...
    return new Response(
      JSON.stringify({ message: 'Assignment restored successfully.' }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      }
